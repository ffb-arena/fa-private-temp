    ...
    constructor(id, x, y, n, name, bruh, lvl, ws) { // n is number of petals
        this.pubInfo = {
            ...
            petals: [
                ...
                new Petal(1, 2 * Math.PI / n, C.normal, { x: x, y: y }),
                new Petal(1, 2 * Math.PI / n * 2, C.normal, { x: x, y: y }),
                new Petal(1, 2 * Math.PI / n * 3, C.normal, { x: x, y: y }),
                new Petal(1, 2 * Math.PI / n * 4, C.normal, { x: x, y: y })
            ],
            face: 0
        }
        this.level = lvl;
    ...
    update(mul, maxX, maxY) {
        ...
        this.pubInfo.petals.forEach(petal => {
            if (this.keys.spaceDown 
                || this.keys.leftMouse) {
                    this.petalDist = Math.min(attack, this.petalDist + C.petalSmooth * mul);
                }
            else if (this.keys.shiftLeft 
                || this.keys.shiftRight 
                || this.keys.rightMouse) {
                    this.petalDist = Math.max(defend, this.petalDist - C.petalSmooth * mul);
                }
            else {
                (this.petalDist < C.normal)
