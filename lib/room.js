...
class Room {
    ...
    _handle1DCollision(p1, p2, mul, notMoving) {
        ...
        players[1].movement.accOffset.x = -players[1].movement.xToAdd / divide * 1.15;
        players[1].movement.accOffset.y = -players[1].movement.yToAdd / divide * 1.15;

        // shortcut since I'm bad at math
        // THIS BUGS SOMETIMES - FIX NEEDED
        players[1].pubInfo.x -= players[1].movement.xToAdd; // ideally should move so both players are just touching
        players[1].pubInfo.y -= players[1].movement.yToAdd; // but idk how

        // in a 1D collision with equal masses and one still object
        // that object gets the other object's velocity
