                ...
                if (!(n <= i)) {
                    ...
                    if (Math.pow(Math.round(d.x * 100) / 100, 2) + Math.pow(Math.round(d.y * 100) / 100, 2)
                        ...
                        if (Math.pow(Math.round(d.x * 100) / 100, 2) + Math.pow(Math.round(d.y * 100) / 100, 2) <= 2500) {  
                            ...
                            const playerAccToAdd = F.coord(
                                (!p.movement.xToAdd && !p.movement.xToAdd) 
                                    ? playerInitialAngle + Math.PI
                                    : avg + (Math.PI * (playerInitialAngle < pInitialAngle)), 
                                0.5);
                            const pAccToAdd = F.coord(
                                (!player.movement.xToAdd && !player.movement.xToAdd) 
                                    ? pInitialAngle + Math.PI
                                    : avg + (Math.PI * (playerInitialAngle > pInitialAngle)), 
                                0.5);
                            ...
                            player.movement.accOffset.x += playerAccToAdd.x * player.movement.acc.x / 1;
                            player.movement.accOffset.y += playerAccToAdd.y * player.movement.acc.y / 1;
                            p.movement.accOffset.x += pAccToAdd.x * p.movement.acc.x / 1;
                            p.movement.accOffset.y += pAccToAdd.y * p.movement.acc.y / 1;
                        }
                    }
                }
            });
