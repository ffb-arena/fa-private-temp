...
class Room {
    ...
    _handle1DCollision(p1, p2, mul, notMoving) {
        let divide = C.acc.flower * C.friction * mul; // divide by this to make x/yToAdd between 0 and 1
        if (notMoving === 1) {
            p2.movement.accOffset.x = -p2.movement.xToAdd / divide * 1.15;
            p2.movement.accOffset.y = -p2.movement.yToAdd / divide * 1.15;

            console.log((p2.pubInfo.x - p1.pubInfo.x));
            // p2.pubInfo.x += 50 - (p2.pubInfo.x - p1.pubInfo.x);
            // p2.pubInfo.y += 100 - (p2.pubInfo.y - p1.pubInfo.y);
        } else {
            p1.movement.accOffset.x = -p1.movement.xToAdd / divide * 1.15;
            p1.movement.accOffset.y = -p1.movement.yToAdd / divide * 1.15;

            console.log((p1.pubInfo.x - p2.pubInfo.x))
            // p1.pubInfo.x += 50 - (p1.pubInfo.x - p2.pubInfo.x);
            // p1.pubInfo.y += 100 - (p1.pubInfo.y - p2.pubInfo.y);
        }
    }
    ...
    _handleBodyCollision(p1, p2, mul) {
        ...
        if (notMoving) {
            this._handle1DCollision(p1, p2, mul, notMoving);
            return;
        }
    }
