                    ...
                    if (Math.pow(Math.round(d.x * 100) / 100, 2) + Math.pow(Math.round(d.y * 100) / 100, 2)
                        ...
                        if (Math.pow(Math.round(d.x * 100) / 100, 2) + Math.pow(Math.round(d.y * 100) / 100, 2) <= 2500) {  
                            ...
                            const playerAccToAdd = F.coord(
                                (!p.movement.xToAdd && !p.movement.yToAdd) 
                                    ? Math.PI + playerInitialAngle
                                    : avg + (Math.PI * (playerInitialAngle < pInitialAngle)), 
                                1
                            );
                            
                            const pAccToAdd = F.coord(
                                (!player.movement.xToAdd && !player.movement.yToAdd) 
                                    ?  Math.PI + pInitialAngle
                                    : avg + (Math.PI * (playerInitialAngle > pInitialAngle)), 
                                1
                            );
                            ...
                            if (player.movement.xToAdd || player.movement.yToAdd) {
                                console.log(playerInitialAngle, playerAccToAdd);
                                player.movement.accOffset.x += playerAccToAdd.x;
                                player.movement.accOffset.y += playerAccToAdd.y;
                            }
                            if (p.movement.xToAdd || p.movement.yToAdd) {
                                console.log(pInitialAngle, pAccToAdd);
