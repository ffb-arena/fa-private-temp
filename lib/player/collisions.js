...
function handleCollision(p1, p2, mul) {
    ...
    const centrePoint = F.centreLine2Circles(
        { x: p1.pubInfo.x, y: p1.pubInfo.y, radius: p1.petalDist },
        { x: p2.pubInfo.x, y: p2.pubInfo.y, radius: p2.petalDist },
        dist.dist
    );
    const vector = F.normalize({ // unit vector of collision
        x: p1.pubInfo.x - p2.pubInfo.x,
        y: p1.pubInfo.y - p2.pubInfo.y
    });
    ...
    const distToCentre = F.pythag(
        p1.pubInfo.x - centrePoint.x,
        p1.pubInfo.y - centrePoint.y
    );
    const distFromCentreToIntersection = F.pythagLeg(p1.petalDist, distToCentre);
    if (Number.isNaN(distFromCentreToIntersection)) return; // change later
    const intersect1 = {
